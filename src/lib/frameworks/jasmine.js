...
const {
  hasStringOrTemplateArgument, getStringValue, getLineNumber, getEndLineNumber, getCode,
} = require('../utils');
  ...
  traverse.default(ast, {
    enter(path) {
      ...
      if (path.isIdentifier({ name: 'fdescribe' })) {
        ...
        throw new CommentError('Exclusive tests detected. `fdescribe` call found in ' + `${file}:${line}\n` + 'Remove `fdescibe` to restore test checks');
      }

      if (path.isIdentifier({ name: 'fit' })) {
        const line = getLineNumber(path);
        throw new CommentError('Exclusive tests detected. `fit` call found in ' + `${file}:${line}\n` + 'Remove `fit` to restore test checks');
      }
