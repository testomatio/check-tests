...
const {
  getStringValue, getLineNumber, getEndLineNumber, getCode, hasStringOrTemplateArgument,
} = require('../utils');
    ...
    enter(path) {
      ...
      if (path.isIdentifier({ name: 'only' })) {
        ...
        if (['Scenario'].includes(name)) {
          ...
          throw new CommentError('Exclusive tests detected. `.only` call found in ' + `${file}:${line}\n` + 'Remove `.only` to restore test checks');
        }
      }
      ...
      if (path.isIdentifier({ name: 'tag' })) {
        if (!path.parentPath.container || !path.parentPath.container.arguments || !path.parentPath.container.arguments[0]) return;
