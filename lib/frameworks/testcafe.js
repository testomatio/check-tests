...
const {
  getLineNumber, getEndLineNumber, getCode, hasTemplateQuasi, hasStringArgument, hasTemplateArgument, getQuasiArgument
} = require('../utils');
      ...
      if (path.isIdentifier({ name: "before" })) {
        ...
        if (path.parent.object.name === 'test') {
          ...
          tests.push({
            ...
            suites: [currentSuite],
            line: getLineNumber(path),
            code: getCode(source, getLineNumber(path.parentPath.parentPath), getEndLineNumber(path.parentPath.parentPath)),
            file,
            skipped: false,
          });
        }
